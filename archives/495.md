...
| result          | `: Promise<TResult>`[^promiseresult] | another promise, first waiting for `this` (the promise) to resolve, *then* calling the argument (the callback) with the resolved value. |

[^promiseresult]: In fact, nested promises in JavaScript are flattened. The real return value here is typed `Promise<Awaited<TResult>>`. In Haskell however, it is possible to construct an `IO (IO ())`, and `join` is needed to unwrap it.
